...
AuthResult AuthService::registerUser(const std::string& name,const std::string& email,const std::string& password,const std::string& role){
    if(name.empty()||email.empty()||password.empty()) return {false,"All fields are required","",User{}};
    if(role!="buyer" && role!="seller") return {false,"Role must be buyer or seller","",User{}};
    if(users_.emailExists(email)) return {false,"Email already registered","",User{}};
    int id=users_.create(name,email,password,role);
    if(id<0) return {false,"Could not create user","",User{}};
...
AuthResult AuthService::login(const std::string& email,const std::string& password){
    if(!users_.validatePassword(email,password)) return {false,"Invalid email or password","",User{}};
